#include "head.h"

void createList(list *L) {
  (*L).first = -1;
  int i;

  for(i=0;i<100;i++){
    // Inisialisasi isi array
    (*L).data[i].next = -2;
  }
}

int countElement(list L){
  int hasil = 0;
  if (L.first != -1){
    // List tak kosong
    int hitung;

    //Init
    hitung = L.first;

    while (hitung != -1){
      // Proses
      hasil = hasil + 1;

      // Iterasi
      hitung = L.data[hitung].next; //= element.next
    }
  }

  return hasil;
}

int emptyElement(list L){
  int hasil = -1;

  if (countElement(L) < 100){
    int ketemu = 0;

    int i = 0;

    while((ketemu == 0) && (i < 100)){
      if (L.data[i].next == -2){
        hasil = i;
        ketemu = 1;
      }
      else{
        i = i + 1;
      }
    }
  }
  return hasil;
}

void addFirst(char nama[], int skala, list *L){
  if(countElement(*L) < 100){
    int baru = emptyElement(*L);

    strcpy((*L).data[baru].kontainer.nama, nama);
    (*L).data[baru].kontainer.skala = skala;
    // printf("add first\n");

    if((*L).first == -1){
      //jika kosong
      (*L).data[baru].next = -1;
    }
    else{
      //Jika isi
      (*L).data[baru].next = (*L).first;
    }
    (*L).first = baru;
  }
  else{
    //Jika array penuh
    printf("sudah tidak dapat ditambah\n");   
  }
}

void addAfter(int prev, char nama[], int skala, list *L){
  if(countElement(*L) < 100){
    int baru = emptyElement(*L);

    strcpy((*L).data[baru].kontainer.nama, nama);
    (*L).data[baru].kontainer.skala = skala;
    // printf("add after\n");
    if((*L).data[prev].next == -1){
      //jika kosong
      (*L).data[baru].next = -1;
    }
    else{
      //Jika isi
      (*L).data[baru].next = (*L).data[prev].next;
    }
    (*L).data[prev].next = baru;
  }
  else{
    //Jika array penuh
    printf("sudah tidak dapat ditambah\n");   
  }
}

void addLast(char nama[], int skala, list *L){
  if((*L).first == -1){
    //jika list kosong
    addFirst(nama, skala, L);
  }
  else{
    //Jika list terisi
    if(countElement(*L) < 100){
      //Jika array belum penuh
      //Mencari element terakhir

      //Init
      int prev = (*L).first;

      while((*L).data[prev].next != -1){
        //iterasi
        prev = (*L).data[prev].next;
      }

      addAfter(prev, nama, skala, L);
      // printf("add last\n");
    }
    else{
      //Jika array penuh
    printf("sudah tidak dapat ditambah\n");
    }
  }
}

void delFirst(list *L){
  if((*L).first != -1){
    int hapus = (*L).first;
    
    if(countElement(*L) == 1){
      (*L).first = -1;
    }
    else{
      (*L).first = (*L).data[hapus].next;
    }
    //elemen awal sebelumnya dikosongkan
    (*L).data[hapus].next = -2;
    // printf("delFirst\n");
  }
  else{
    //Jika list masih kosong
    printf("list kosong\n");
  }
}

void delAfter(int prev, list *L){
  int hapus = (*L).data[prev].next;

  if(hapus != -1){
    if((*L).data[hapus].next == -1){
      (*L).data[prev].next = -1;
    }
    else{
      (*L).data[prev].next = (*L).data[hapus].next;
    }
    //pengosongan elemen
    (*L).data[hapus].next = -2;
    // printf("delAfter\n");
  }
}

void delLast(list *L){
  if((*L).first != -1){
    if(countElement(*L) == 1){
      //Jika ist isi 1 elemen
      delFirst(L);
    }
    else{
      int hapus = (*L).first;
      int prev;

      while((*L).data[hapus].next != -1){
        //iterasi
        prev = hapus;
        hapus = (*L).data[hapus].next;
      }
      //elemen sebelum terakhir diganti jadi terakhir
      delAfter(prev, L);
    }
    // printf("delLast\n");
  }
  else{
      //Jika list kosong
      printf("list kosong\n");
  }
}

void sortAndPrintElement(int param, list L){
  if(L.first != -1){
    //init
    pasien temp[countElement(L)];
    pasien store;
    int tunjuk = L.first,
        noUrut = 1,
        move = 0,
        i = 0,
        min = L.data[L.first].kontainer.skala,
        max = L.data[countElement(L) - 1].kontainer.skala;

    while(tunjuk != -1){
      strcpy(temp[i].nama, L.data[tunjuk].kontainer.nama);
      temp[i].skala = L.data[tunjuk].kontainer.skala;
      //next
      tunjuk = L.data[tunjuk].next;
      i++;
    }

    do{
      move = 0;

      for(i = 0; i < countElement(L) - 1; i++){
        if(param == 1){
          if(temp[i+1].skala < temp[i].skala){
            strcpy(store.nama, temp[i].nama);
            store.skala = temp[i].skala;

            strcpy(temp[i].nama, temp[i+1].nama);
            temp[i].skala = temp[i+1].skala;

            strcpy(temp[i+1].nama, store.nama);
            temp[i+1].skala = store.skala;

            move++;
          }
        }else{
          if(temp[i+1].skala > temp[i].skala){
            strcpy(store.nama, temp[i].nama);
            store.skala = temp[i].skala;

            strcpy(temp[i].nama, temp[i+1].nama);
            temp[i].skala = temp[i+1].skala;

            strcpy(temp[i+1].nama, store.nama);
            temp[i+1].skala = store.skala;

            move++;
          }
        }
      }
    }while(move != 0);
    

    for(i = 0; i < countElement(L); i++){
      printf("%d. %s\n", noUrut, temp[i].nama);
      noUrut++;
    }
  }
  else{
    //Jika list kosong
    printf("list kosong\n");
  }
  
}

void delAll(list *L){
  int i;

  for(i = countElement(*L);i>=1;i--){
    //menghapius elemen
    delLast(L);
  }
}
